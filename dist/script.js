...
window.addEventListener('DOMContentLoaded', function () {
  ...
  var modalState = {}; // состояние модального окна где что-то выберается
...
var checkNumInputs = function checkNumInputs(selector) {
  var numInputs = document.querySelectorAll(selector); // Валидация
...
var forms = function forms(state) {
  ...
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_5__["default"])('input[name="user_phone"]'); // Объект для вывода сообщения о процессе
    ...
    return regeneratorRuntime.async(function postData$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            // с помощью есинк джс узнает о ассинхронной операции
            ...
            return regeneratorRuntime.awrap(fetch(url, {
              // с помощью евейт джс ждет окончания отправки запроса и получаем результат в res
              method: 'POST',
              body: data
            }));
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      var formData = new FormData(item); // Собираем все данные в форме с помощью FormData()

      if (item.getAttribute('data-calc') === "end") {
        // Ищем форму с калькулятором
      ...
      postData('assets/server.php', formData).then(function (res) {
        ...
        // выполнится в любом случае результата промиса
        clearInputs(); // очистка всех инпутов

        setTimeout(function () {
          // удаляем созданный блок через некоторое время
...
var timer = function timer(id, deadline) {
  ...
  var setClock = function setClock(selector, endtime) {
    ...
    updateClock(); // функция апдейта времени
    ...
    function updateClock() {
      ...
      seconds.textContent = addZero(t.seconds); // если тотал время не больше 0

      if (t.total <= 0) {
        days.textContent = "00";
        hours.textContent = "00";
        minutes.textContent = "00";
        seconds.textContent = "00"; // останавливаем и очищаем интервал
